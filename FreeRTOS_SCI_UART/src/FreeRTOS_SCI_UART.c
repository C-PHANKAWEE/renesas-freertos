...
void sender_task(void *pv){
	while(1){
		R_SCI_Send(g_my_sci_handle, "Hello\n", 6);
...
void receiver_task(void *pv){
	...
	while(1){
		...
		do
		{
			my_sci_err = R_SCI_Receive(g_my_sci_handle, &my_char, 5);
		} while (SCI_ERR_INSUFFICIENT_DATA == my_sci_err);

		sprintf((char *)my_str, "Message Received : %s\r\n", &my_char);
...
void main_task(void *pvParameters)
{
	...
	xTaskCreate(sender_task, "Sender Task", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY + 1, NULL);
